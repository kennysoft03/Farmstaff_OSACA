...
class Mailtemplete {
    ...
    public function __construct() {
        ...
        //For Mailing Purpose
        define('SITE_EMAIL_FROM',       'admin@giftrete.com');
        ...
        //define('MAILIN_PASSWORD',  'GiftRete!23');
    ...
    public function send_mail($from = "", $to = '', $templete_type = '', $param = array()) {
        ...
        // PHPMailer object
        $mailer = $CI->phpmailer->load();

        // SMTP configuration
       /* $mailer->SMTPDebug = 0;
        $mailer->isSMTP();
        $mailer->Host     = 'giftrete.org';
        $mailer->SMTPAuth = true;
        $mailer->Username = 'postmaster@giftrete.org';
        $mailer->Password = '2[I_K$@oAW.1w%QB';
        ...
        $mailer->SMTPDebug = 0;
        $mailer->isSMTP();
        $mailer->Host     = 'giftrete.com';
        $mailer->SMTPAuth = true;
        $mailer->Username = 'postmaster@giftrete.com';
        $mailer->Password = '2[I_K$@oAW.1w%QB';
        $mailer->SMTPSecure = 'ssl';
        $mailer->Port     = 465;
        ...
        $replyTo    = SITE_EMAIL_REPLYTO;
         ...
         //For pram name for  Email -08062096811 - Daddy Mary
        $name = '';
        ...
        #$CI->phpmailer->addAddress($to, $name);
        $mailer->addAddress($to);
        $mailer->setFrom(SITE_EMAIL_FROM, SITE_TITLE);
        $mailer->addReplyTo(SITE_EMAIL_REPLYTO, SITE_TITLE);
        

        if(defined('SITE_EMAIL_BCC')){
            $mailer->addBCC(SITE_EMAIL_BCC);
        }
        ...
        try{
            ...
            $send="Message could not be sent. Mailer Error: {$mailer->ErrorInfo}";
        }
