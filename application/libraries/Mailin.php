...
class Mailin
{
  ...
  {
    $this->username = MAILIN_USERNAME;
    $this->password = MAILIN_PASSWORD;
    
    // $this->username = 'admin@giftrete.com';
    // $this->password = '79bVBT6QFOSMm1Hg';
 ...
 /**
  ...
  public function setTo($email, $name=null)
  {
        if(!empty($name)) {
	        ...
	        $this->to_list[] = array('email' => $email, 'name' => $email);    
	    }
  ...
  public function getFrom($as_array = false)
  {
    if($as_array && ($name = $this->getFromName())) {
      return array('email' => $this->from, 'name' => $this->from_name);
  ...
  public function setBcc($email){
    $this->bcc_list = array('email' => $email,'name' => $email);
    return $this;
  }
   ...
   */
    public function send()  {
        ...
        $newparams = array(
          'tag'         => $tag,
          'sender'      => $this->getFrom(true),
          'htmlContent' => $this->getHtml(),
          'textContent' => $this->getSubject(),
...
protected function _prepMessageData__()  {
    $params = 
    array(
